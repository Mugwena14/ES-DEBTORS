...
const RequestForm = ({ 
  ...
  return (
    <div 
      className="bg-[#111827] p-8 shadow-2xl border-b-4" 
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-2">
          ...
          <input
            type="text"
            className="w-full bg-white/5 border border-gray-700 p-3 text-white outline-none font-mono focus:border-[var(--focus-color)]"
            style={{ "--focus-color": accentColor }}
            placeholder="e.g. 9201015000081"
            value={clientId}
            onChange={(e) => setClientId(e.target.value)}
            onFocus={(e) => e.target.style.borderColor = accentColor}
            onBlur={(e) => e.target.style.borderColor = '#374151'} // gray-700
          />
        </div>
        <div className="space-y-2">
          <label className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Select Creditor</label>
          <select
            className="w-full bg-white/5 border border-gray-700 p-3 text-white outline-none appearance-none"
        ...
        <div className="flex items-end">
          <button
            ...
            className="w-full text-white font-black py-3 uppercase tracking-tighter hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2 disabled:bg-gray-800 disabled:text-gray-600"
            style={{ 
